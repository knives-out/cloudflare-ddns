    ...
    if ipv4_enabled:
        try:
            ...
            if not shown_ipv4_warning:
                ...
                print("🧩 IPv4 not detected via ip.sb, trying 1.0.0.1")
            # Try secondary IP check
            try:
                a = requests.get(
                    "https://ifconfig.me").text.split("\n")
                a.pop()
                a = dict(s.split("=") for s in a)["ip"]
            except Exception:
                ...
                if not shown_ipv4_warning_secondary:
                    ...
                    print("🧩 IPv4 not detected via ifconfig.me. Verify your ISP or DNS provider isn't blocking Cloudflare's IPs.")
